...
class DataNum:
    ...
    def input_num(self):
        while True:
            ...
            num = input('ведите целое число и нажите Enter для продолжения,'
                        'или наберите stop для выхода ')
            if num.lower() == 'stop':
                print(f'вы закончили ввод, введеный список  {self.num_list}')
                break
            else:
                try:
                    num = int(num)
                    self.num_list.append(num)
                    print(f' введеный список:  {self.num_list}')
                except:
                        print(f' введенные данные - не число')

user_list = DataNum(2)
print(user_list.input_num())